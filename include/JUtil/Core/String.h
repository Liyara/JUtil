...
#include <cwchar>
    ...
    #define JUTIL_ENFORCE_TYPE_STRING_CONVERSION(T) = typename Enable<Convert<T, String>::Value, T>::Type
    #define JUTIL_ENFORCE_STRING_TYPE_CONVERSION(T) = typename Enable<Convert<String, T>::Value, T>::Type
...
#define __STRING_DO_CONVERSION__(type, func, ret)        \
    type *__arr__ = new type[size() + 1];                \
    array(__arr__);                                      \
    ret __val__ = cstr_conversion::strTo##func(__arr__); \
    delete[] __arr__;                                    \
    return __val__
...
#define __STRING_QUEUE_OVERRIDES__(type)                                                                                                                                                                             \
    bool find(const StringBase<type> &q, size_t start, size_t end, size_t start2, size_t end2, size_t *p = JUTIL_NULLPTR) JUTIL_C_ { return Queue<type>::find(q, start, end, start2, end2, p); }                     \
    bool find(const StringBase<type> &q, size_t start, size_t end, size_t start2, size_t *p = JUTIL_NULLPTR) JUTIL_C_ { return Queue<type>::find(q, start, end, start2, p); }                                        \
    bool find(const StringBase<type> &q, size_t start, size_t end, size_t *p = JUTIL_NULLPTR) JUTIL_C_ { return Queue<type>::find(q, start, end, p); }                                                               \
    bool find(const StringBase<type> &q, size_t start, size_t *p = JUTIL_NULLPTR) JUTIL_C_ { return Queue<type>::find(q, start, p); }                                                                                \
    bool find(const StringBase<type> &q, size_t *p = JUTIL_NULLPTR) JUTIL_C_ { return Queue<type>::find(q, p); }                                                                                                     \
    bool replace(const StringBase<type> &value, const StringBase<type> &value2, size_t start, size_t end, size_t start2, size_t end2) { return Queue<type>::replace(value, value2, start, end, start2, end2); }      \
    bool replace(const StringBase<type> &value, const StringBase<type> &value2, size_t start, size_t end, size_t start2 = 0) { return Queue<type>::replace(value, value2, start, end, start2); }                     \
    bool replace(const StringBase<type> &value, const StringBase<type> &value2, size_t start = 0) { return Queue<type>::replace(value, value2, start); }                                                             \
    bool replaceFirst(const StringBase<type> &value, const StringBase<type> &value2, size_t start, size_t end, size_t start2, size_t end2) { return Queue<type>::replace(value, value2, start, end, start2, end2); } \
    bool replaceFirst(const StringBase<type> &value, const StringBase<type> &value2, size_t start, size_t end, size_t start2 = 0) { return Queue<type>::replace(value, value2, start, end, start2); }                \
    bool replaceFirst(const StringBase<type> &value, const StringBase<type> &value2, size_t start = 0) { return Queue<type>::replace(value, value2, start); }
...
namespace jutil  {
    ...
    template <typename T>
    class  StringBase : public Queue<T> {
       public:
        StringBase() {}
        StringBase(const Queue<T> &) {}
        template <typename U>
        StringBase(const StringBase<U> &) {}
        StringBase(T) {}
        template <size_t l>
        StringBase(const T (&c)[l]) {}
    ...
    template <>
    class  StringBase<char> : public Queue<char> {
       public:
        ...
        JUTIL_EXPL_ operator bool() JUTIL_CN_ {
            ...
            upperCase(jutil::trim(o));
            if (o == StringBase<char>("FALSE") || o == StringBase<char>("0"))
                return false;
            else
                return true;
        }
        ...
        __STRING_QUEUE_OVERRIDES__(char)
    };
    ...
    template <>
    class  StringBase<wchar_t> : public Queue<wchar_t> {
       public:
        ...
        JUTIL_EXPL_ operator bool() JUTIL_CN_ {
            ...
            upperCase(jutil::trim(o));
            if (o == StringBase<wchar_t>(L"FALSE") || o == StringBase<wchar_t>(L"0"))
                return false;
            else
                return true;
        }
        ...
        __STRING_QUEUE_OVERRIDES__(wchar_t)
    };
    ...
    template <>
    class  StringBase<Byte> : public Queue<Byte> {
       public:
        StringBase() {}
        ...
        __STRING_QUEUE_OVERRIDES__(Byte)
    };
...
inline bool operator==(const jutil::StringBase<T> &l, const jutil::StringBase<U> &r) {
    if (l.size() == r.size()) {
        for (size_t i = 0; i < l.size(); ++i)
            if (l[i] != static_cast<T>(r[i])) return false;
        return true;
    } else
        return false;
}
